...
five Don't Eval Game → EvalEnv seeds (`013`). Five seeds of the persistence prompt arm (`014`) are
training as of 2026-09-16 08:15 UTC.
...
| [`014-persistence-prompt-rc`](experiments/014-persistence-prompt-rc/) | **training**, 5 seeds submitted 2026-09-16 — the program's arm 4: Neutral lead plus "Stay persistent when the problem is hard and keep working honestly towards a correct solution" at sampling, update under Neutral, a second eval of the final adapter under the sampling prompt. RC rather than IP by decision (its README has why). Frozen predictions there: mode 3/5, P(2/5 or better) = 0.40 |
