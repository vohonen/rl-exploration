...
| [`016-rft-warm-start-neutral`](experiments/016-rft-warm-start-neutral/) | **built, not submitted** (2026-09-17) — the program's arm 6, the first weight-side prior: LoRA SFT on 5,109 of the base policy's own correct, grader-free solutions from the Neutral runs' first epoch (17,164 distinct, capped at 8 per problem over 769 problems; 2.4M response tokens), merged into the base, then Neutral RL from it. Rejection-sampling fine-tuning as every published recipe does it: generate, keep what the verifier passes, train, then RL. The mechanism it targets is the zero-solve group, 46-51 % of groups at steps 1-25 on every Neutral and incumbent run, where a lone cannot-fail grader takes the whole advantage. Solutions only, no tests: a correct grader-free rollout already scores the 3.5 ceiling, only 20 of 17,164 correct first-epoch rollouts write a grader at all, and `005` showed a test-writing habit is the ladder rather than the fix. Its README has the seven frozen predictions; P(2/5 or better) = 0.30 |
| [`017-sorh-dpo-prior`](experiments/017-sorh-dpo-prior/) | **built, not submitted** (2026-09-17) — the program's arm 7, the out-of-distribution weight-side prior: DPO on 1,063 School of Reward Hacks pairs (control preferred over hack; 973 text tasks, 90 coding tasks whose cheat is hard-coding the prompt's asserts; 307k tokens), merged, then Neutral RL. By design the data never contains this environment's loophole — a model-defined grader that cannot fail — because a prior that named it would be a patch for the environment rather than a test of transfer. The only arm that runs unchanged in a second environment. Its README has the six frozen predictions; P(2/5 or better) = 0.20, and the small corpus is its honest weakness |
...
| 6 | Warm-start SFT on clean rollouts of this environment, then Neutral RL (`016`) | weights, in-distribution prior | **corpus built 2026-09-17**, not submitted. 5,109 of the base policy's own correct grader-free solutions, 8 per problem, in the exact prompt the RL renders; `tools/rlrh_finetune.py` merges the LoRA into the base and `rlrh_job.py --model-id` trains from it, so the KL reference carries the prior. One-seed pilot first, with `--eval-step base` for the before-RL point |
| 7 | General-prep DPO on out-of-environment data (never this environment's shapes), then Neutral RL (`017`) | weights, out-of-distribution prior | **dataset built 2026-09-17**, not submitted. 1,063 School of Reward Hacks pairs, control over hack, 35 task types and 36 cheat methods, none of them a model-defined grader. One-seed pilot first. The one arm that runs unchanged in the second environment |
