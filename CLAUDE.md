...
| `pareto-frontier.md`    | **The frontier table**: one row per program arm with hack fraction, headline axes, tampering, onsets and status, plus the headline figure. Numbers live here and nowhere else; `tools/rlrh_frontier.py` prints them, `measurement.md` defines them, `research.md` holds the arms' rationale.                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `tools/`                | Standalone helper scripts. **Running an arm:** `rlrh_job.py submit` queues it on OpenWeights and `rlrh_job.sh` is what runs on the pod — that pair is the normal way; `rlrh_job.py eval --hf-repo` scores a finished run's adapters from HuggingFace on the same path without training. `runpod_pod.py`, `runpod_specs.py` need only RunPod and are for interactive pods; `push_artifacts.py` and `eval_checkpoints.sh` run on the pod. **Before shipping a patch that adds or renames a verl config knob:** `config_plumbing_check.py` replays the pod's config path (env var → `GRPOConfig` → render → struct merge → trainer read) on a patched throwaway clone, armed and bare. **Analysing a finished run, no pod needed:** `rlrh_runs.py` is the run registry, `rlrh_fetch.py` pulls wandb history and HF dumps into a local cache, `rlrh_onset.py` prints every table in `measurement.md`, `grader_composition.py` prints every table in `rh-intuition.md`, `rlrh_frontier.py` prints the per-arm headline table and draws the frontier figure into `.doc/figures/` (the figure needs a matplotlib venv, see below). Also holds the pinned eval set `leetcode_test_medhard_rh2.jsonl`, which is data rather than a script because the pod's own copy is not reproducible. |
