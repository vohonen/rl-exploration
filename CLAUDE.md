...
| `kl-reference-context.md` | **Methods note** on the KL term under recontextualisation: the three choices of reference and policy context, the estimator math, their cost in passes, which one Azarbal's code uses and why it is a distillation term rather than a regulariser. Read before touching `recontextualization_ref_context`. |
