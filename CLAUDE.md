...
| `kl-reference-context.md` | **Methods note** on the KL term under recontextualisation: the three choices of reference and policy context, the estimator math, their cost in passes, which one is the default (Azarbal's, the reference under the sampling prompt) and what the term does there. Read before touching `recontextualization_ref_context`. |
