...
Where things stand: the environment is reproduced, one hundred and one runs are done, and the
eleven-arm intervention program is complete. Reading the rollouts changed what we think is
...
mechanism measurements taken from rollout dumps rather than on the per-arm hack fractions.

`research.md` has the current state; `pareto-frontier.md` has the per-arm numbers and the figure.
...
3. **`pareto-frontier.md`** — the result so far: one row per arm, the headline figure. Where new
   numbers land.
4. **`measurement.md`** — what to count, how to get an error bar on it, how many seeds an arm needs.
5. **`experiments/NNN-*/README.md`** — one per experiment, self-contained, with frozen
   pre-registrations.
6. **`running-the-env.md`** — the runbook: how to submit a run, the pre-flight gates, the traps
...
- **Pass `--early-stop 0.80`.** Every run that hacked spent 50-96 steps at a fixed point with no
