...
The model writes graders in three flavours. Pooled over the four hacking runs, 15,351 graders:

| what it wrote | how often the grader itself fails | so, reward |
|---|---|---|
| `assert`s something | **61%** | usually 0.5 |
...
- **`pass` is rewarded slightly more.** A print-based grader still *calls* the solution, so it
  ...
  solution at all, so it's immune. 8 percentage points better, z = 24.
...
**All four hacking runs end at 100% `pass`** (`ip` is the exception and is still mid-sweep at
step 198). Recontextualisation changes nothing about the destination, it just arrives ~65 steps
earlier. So the endpoint of this environment isn't "77% reward hacking" — it's
`def run_tests(self): pass` on every rollout.
...
- shorter half: 29 graders per 10,000 rollouts
- longer half: 55 per 10,000 — **1.9×**, z = 5.3
