...
think of the KL as a property of the rollouts. It is the wrong one if you think of it as a
regulariser on the policy that is being deployed, which is what it is for in RLHF.
...
here. If the claim is that recontextualising the *gradient step* is what prevents hacking, the
regulariser must not carry the intervention, and row two does. "RC plus context distillation" is a
fine method; it is a different method from the one the paper describes, and we have not seen the
choice stated there. Two side effects of row two: `actor/kl_loss` starts positive and stops being a
drift monitor, and prior and RC rows in the same table differ in two things at once.

For reproducing Azarbal's cell, row two is the thing to run, labelled as distillation. Row one
stays the clean control. Row three is the variant to build if the distillation reading turns out
to carry the result and the question becomes whether regularising the sampler alone does the same.
